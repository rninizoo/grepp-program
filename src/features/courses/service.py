...
def create_course(course_create: CourseCreate, actant_id: int,session: Session) -> Course:
    # title 중복 체크
    existing_course = session.exec(select(Course).where(Course.title == course_create.title, Course.isDestroyed.is_(False))).first()
    if existing_course:
...
def find_courses(session: Session, skip: int, limit: int, query_opts: CourseQueryOpts = Depends()) -> list[CourseRead]:
...
def update_course(course_id: int, course_update: CourseUpdate, actant_id :int, session: Session) -> CourseRead:
...
def bulk_update_course(course_updates: list[tuple[int, CourseUpdate]], actant_id: int, session: Session):
    try:
        with session.begin():
            results: list[CourseRead] = []
            for course_id, course_update in course_updates:
                updated_course = update_course(course_id, course_update, actant_id, session)
