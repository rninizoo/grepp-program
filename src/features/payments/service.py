...
class PaymentService:
    ...
    def find_payment_by_id(self, id: int, session: Session) -> Payment | None:
        statement = select(Payment).where(Payment.id == id, Payment.isDestroyed.is_(False))
        ...
        try:
            with session.begin():
                ...
                for payment_id, payment_update in payment_updates:
                    updated_payment = self.update_payment(payment_id, payment_update, user_id, session)
