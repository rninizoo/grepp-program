...
def create_payment(payment_create: PaymentCreate, user_id: int, session: Session) -> Payment:
    # validFrom, validTo 검사
    if payment_create.validFrom >= payment_create.validTo:
        raise HTTPException(status_code=400, detail="Invalid validFrom/validTo range")
    ...
    existing_payment = session.exec(
        select(Payment).where(
            ...
            Payment.amount == payment_create.amount,
...
def update_payment(payment_id: int, payment_update: PaymentUpdate, user_id: int, session: Session) -> PaymentRead:
...
def bulk_update_payment(payment_updates: list[tuple[int, PaymentUpdate]], user_id: int, session: Session):
    try:
        with session.begin():
            results: list[PaymentRead] = []
            for payment_id, payment_update in payment_updates:
                updated_payment = update_payment(payment_id, payment_update, user_id, session)
